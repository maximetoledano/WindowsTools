...
# ==> $Date: 22/10/03 9:42 $
# ==> $Author: Mtoledano $
# ==> $Archive: /Scripts Perl/ADP/extract_depends_class.pl $
# ____________________________________________________________
...
my $path_tmp = $ENV{ADPTempDir};
my $path_src = $ENV{ADPFichUses};

if ($path_tmp eq "")
{ $path_tmp = "C:\Temp"; }

my $path_result = "$path_tmp/DependsClassesRose.txt";
...
{ die "$nom_cmd: Ouverture fichier des utilisations Rose incorrecte ($path_src)\n"; }

my $hdl_result = new IO::File $path_result, O_WRONLY|O_CREAT;
unless (defined $hdl_result)
{ die "$nom_cmd: Creation fichier des dependances incorrecte ($path_result)\n"; }
